...
All documentation lives in `/docs/` except four root-level files:
...
Issues, plans, epics, and PRDs live in `.scratch/` — managed by the engineering skills (`/to-prd`, `/to-issues`). Do not create a `docs/backlog/` or `docs/completed/` folder.
...
Files in `/docs/architecture/` follow numeric prefixes for ordering:
...
touch docs/architecture/170_caching_strategy.md

# 2. Add metadata header
cat >> docs/architecture/170_caching_strategy.md << 'EOF'
