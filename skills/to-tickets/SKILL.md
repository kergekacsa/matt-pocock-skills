...
Present the proposed breakdown as a markdown table so the same block can be reused in step 6. For each ticket, use these columns:

| Issue | Title | Type | Label | What it delivers | Blocked by | User stories covered |
|-------|-------|------|-------|------------------|------------|----------------------|
...
### 6. Append the dependency map to the spec ticket


Only do this if the source was an existing spec / parent issue (i.e. the user passed an issue reference in step 1). If the breakdown came from raw conversation context with no parent ticket, skip this step — there is nothing to append to.

Run this AFTER the tickets are published, so the table can reference real ticket identifiers.

Append a `## Dependency map` section to the **end** of the spec ticket's body. Use a story-traceability variant of the step 4 table — **add** the `User stories` column (mapped against the spec's own user-story list, so orphan stories become visible) and **drop** the `Label` and `What it delivers` column:
...
- Do NOT alter the rest of the spec body, change its status, or close it.
