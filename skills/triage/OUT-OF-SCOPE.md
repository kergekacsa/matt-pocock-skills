...
Only when an **enhancement** (not a bug) is *rejected* as `wontfix`. This applies to enhancement PRs exactly as it does to tickets — a rejected PR is recorded here so the same request doesn't return as fresh code.
