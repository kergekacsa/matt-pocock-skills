...
"explain this codebase to a new developer", use `onboarding-guide.md` instead of
`architecture-guide.md` below. This mode explains rather than diagnoses — no Health
Score, no Iron Law findings.
...
Delegate the audit to one `general-purpose` subagent with `model: opus`. It starts
with a fresh context, so give it the scope and tell it to read for itself:
`../_shared/common.md`, `../_shared/source-coverage.md`, `../_shared/decay-risks.md`,
then `architecture-guide.md` in this directory (or `onboarding-guide.md` in onboarding
mode). Its brief:
...
- **Onboarding mode:** follow `onboarding-guide.md`'s process and Output Template end to end.

Present the subagent's report verbatim.
